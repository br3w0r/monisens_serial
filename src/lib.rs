...
const CONN_PARAM_SERIAL: &str = "Serial Port";
...
pub struct Module {
    ...
    serial: Option<Arc<Mutex<Box<dyn serialport::SerialPort>>>>,
...
pub unsafe extern "C" fn obtain_device_info(
    ...
    let choices_raw: Vec<CString> = ports
        .iter()
        .map(|v| {
            let typ = match v.port_type {
            ...
            let res = v.port_name.clone() + " | " + typ;

            CString::new(res).unwrap()
        })
        .collect();

    let port_param_name = CString::new(CONN_PARAM_SERIAL).unwrap();
    let port_param_chioces_vec: Vec<*mut u8> =
        choices_raw.iter().map(|v| v.as_ptr() as _).collect();
    let port_param_chioces = bg::ConnParamChoiceListInfo {
        choices: port_param_chioces_vec.as_ptr() as _,
        chioces_len: port_param_chioces_vec.len() as _,
    };

    let params_vec: Vec<bg::ConnParamInfo> = vec![bg::ConnParamInfo {
        name: port_param_name.as_ptr() as _,
        typ: bg::ConnParamType::ConnParamChoiceList,
        info: &port_param_chioces as *const bg::ConnParamChoiceListInfo as *mut c_void,
    }];
    ...
    module.serial_ports = Some(ports.iter().map(|v| v.port_name.clone()).collect());
...
fn connect_device_impl(
    ...
    let ports = module
        .serial_ports
        .as_ref()
        .ok_or(DeviceErr::DeviceErrParams)?;
    let conf = ConnConf::new(confs)?;

    if conf.port > ports.len() - 1 {
        return Err(DeviceErr::DeviceErrParams);
    }

    let serial = serialport::new(&ports[conf.port], 9600)
    ...
    module.serial = Some(Arc::new(Mutex::new(serial)));
...
impl ConnConf {
    fn new(confs_raw: *mut bg::DeviceConnectConf) -> Result<Self, DeviceErr> {
        ...
        if confs.len() != 1 || name != CONN_PARAM_SERIAL {
            return Err(DeviceErr::DeviceErrParams);
        }

        let port =
            c_parser::as_from_str::<usize>(confs[0].value).ok_or(DeviceErr::DeviceErrParams)?;
